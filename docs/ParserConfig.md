...
Парсер обновляет файл `bin/config.json`, загружая подписки VLESS/VMess/Trojan/Shadowsocks, фильтруя и группируя их в селекторы. Результат записывается в секцию между маркерами `/** @ParserSTART */` и `/** @ParserEND */`.
...
|---------------|----------|--------------|----------|
| `source`      | string   | Да           | URL VLESS/VMess/Trojan/Shadowsocks подписки. Допускаются Base64 и plain-текст. |
| `connections` | array    | Нет          | Массив прямых ссылок (vless://, vmess://, trojan://, ss://). Можно комбинировать с подписками. |
| `skip`        | array    | Нет          | Список фильтров. Если хотя бы один совпал — узел пропускается. |
| `tag_prefix`  | string   | Нет          | Префикс, добавляемый ко всем тегам узлов из этого источника (версия 4). Применяется перед оригинальным тегом. |
| `tag_postfix` | string   | Нет          | Постфикс, добавляемый ко всем тегам узлов из этого источника (версия 4). Применяется после оригинального тега. |
| `outbounds`   | array    | Нет          | Локальные outbounds для этого источника (версия 4). Применяются только к узлам из этого источника. |

#### Префиксы и постфиксы тегов (версия 4)

Поля `tag_prefix` и `tag_postfix` позволяют автоматически добавлять префикс и постфикс ко всем тегам узлов из конкретного источника. Это полезно для:
...
При использовании визарда конфигурации, если в поле URL введено несколько подписок (больше одной), визард автоматически добавляет `tag_prefix` с порядковым номером в формате `"1:"`, `"2:"`, `"3:"` и т.д. для каждой подписки. Это происходит только при наличии нескольких подписок - для одной подписки префикс не добавляется автоматически.

**Порядок применения:**
1. Узел парсится с оригинальным тегом (например, `"🇷🇺 Moscow"`)
2. Применяется `tag_prefix` (если указан): `"1:🇷🇺 Moscow"` или `"source1-🇷🇺 Moscow"`
3. Применяется `tag_postfix` (если указан): `"1:🇷🇺 Moscow--xx"` или `"source1-🇷🇺 Moscow--xx"`
...
Если узел имел тег `"🇷🇺 Moscow"`, итоговый тег будет:
- С автоматическим префиксом: `"1:🇷🇺 Moscow"`
- С ручным префиксом и постфиксом: `"source1-🇷🇺 Moscow--xx"`
